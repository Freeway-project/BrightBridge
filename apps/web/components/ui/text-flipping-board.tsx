...
const BASE_TOTAL_S =
  ((DEFAULT_BOARD_COLS - 1) * BASE_COL_DELAY +
    (DEFAULT_BOARD_ROWS - 1) * BASE_ROW_DELAY +
    8 * BASE_STEP_MS) /
  1000;
...
export function TextFlippingBoard({
  ...
  text,
  className,
  rowCount = DEFAULT_BOARD_ROWS,
  columnCount = DEFAULT_BOARD_COLS,
  ...
  const board = useMemo(() => {
    ...
    if (text) {
      const lines = wrapText(text, columnCount).slice(0, rowCount);
      ...
      lines.forEach((line, i) => {
        ...
        const startCol = Math.max(
          0,
          Math.floor((columnCount - parsed.length) / 2),
        );
        parsed.forEach((cell, c) => {
          if (startCol + c < columnCount) {
    ...
    return grid;
  }, [columnCount, rowCount, rows, text]);
